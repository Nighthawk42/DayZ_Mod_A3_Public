...
//server only
if (isServer) then {
	call compile preprocessFileLineNumbers "\dayz_server\init\publicEH_server.sqf";
};

//clients only
if (!isDedicated) then {
//	["norrnRaLW",   		{ [_this select 1] execVM "\dayz_code\medical\publicEH\load_wounded.sqf"; }] call registerBroadcastRpc;
//	["norrnRLact",			{ [_this select 1] execVM "\dayz_code\medical\load\load_wounded.sqf"; }	] call registerBroadcastRpc;	
...
//	["norrnRaDrag",			{ [_this select 1] execVM "\dayz_code\medical\publicEH\animDrag.sqf"; }	] call registerBroadcastRpc;
};
