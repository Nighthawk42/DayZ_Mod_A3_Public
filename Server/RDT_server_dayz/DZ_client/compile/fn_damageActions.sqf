	...
	if (!r_player_unconscious && !r_action2) then {
		...
		if (count _assignedRole > 1) then {
			...
			_weapons = _vehicle weaponsTurret _turret;
			{
				...
				_action = _vehicle addAction [format["Add AMMO to %1",_weaponName], "\dayz_code\actions\ammo.sqf",[_vehicle,_x,_turret], 0, false, true];
...
if (_hasPatient and !r_drag_sqf and !r_action and !_inVehicle and !r_player_unconscious and _isClose) then {
	...
	if(_unconscious) then {
		...
		_action2 = _unit addAction [localize "str_actions_medical_02", "\dayz_code\medical\pulse.sqf",_unit, 0, true, true];
	...
	if (_hasVehicle and _unconscious) then {
		...
		_action = _unit addAction [format[localize "str_actions_medical_03",_vehType], "\dayz_code\medical\load\load_act.sqf",[player,_vehicle,_unit], 0, true, true];
	...
	if(_injured and _hasBandage) then {
		r_action = true;
		//_unit setdamage 0.8;
		_action = _unit addAction [localize "str_actions_medical_04", "\dayz_code\medical\bandage.sqf",[_unit], 0, true, true, "", "'ItemBandage' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Epinephrine
	if(_unconscious and _hasEpi) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_05", "\dayz_code\medical\epinephrine.sqf",[_unit], 0, true, true];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Morphine
	if((_legsBroke or _armsBroke) and _hasMorphine) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_06", "\dayz_code\medical\morphine.sqf",[_unit], 0, true, true, "", "'ItemMorphine' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Painkillers
	if(_inPain and _hasPainkillers) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_07", "\dayz_code\medical\painkiller.sqf",[_unit], 0, true, true, "", "'ItemPainkiller' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to transfuse blood
	if(_lowBlood and _hasBlood) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_08", "\dayz_code\medical\transfusion.sqf",[_unit], 0, true, true, "", "'ItemBloodbag' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	...
	if ((_unit isKindOf "AllVehicles") and !(_unit isKindOf "Man")) then {
		...
		if ((fuel _unit < 1) and _hasJerry) then {
			r_action = true;
			_action = _unit addAction [format[localize "str_actions_medical_10",_typeVeh],{params ["_target", "_caller", "_actionId", "_arguments"];(_target) spawn RT_fnc_refuel;},nil, 0, true, true, "", "'ItemJerrycan' in magazines player"];
			r_player_actions set [count r_player_actions,_action];
		};		
		/*
			...
			_magTypes = getArray(configFile >> "cfgWeapons" >> _weapon >> "magazines");
			{
				...
				if (_ammoSerial > -1) then {
					...
					if (_ammoQty > 0) then {
						...
						_action = _unit addAction [format[localize "str_actions_medical_11",_displayName], "\dayz_code\actions\logistics_loadmag.sqf",[_unit,_x], 0, false, true];
						r_player_actions set [count r_player_actions,_action];
					};
				};
			} forEach _magTypes;
		...
		if ((_type in USEC_PackableObjects) and (_classbag == "")) then {
			r_action = true;
			_action = _unit addAction [format[localize "str_actions_medical_12",_typeVeh], "\dayz_code\actions\pack.sqf",[_unit], 0, true, true];
			r_player_actions set [count r_player_actions,_action];
		};
		*/
	};
	...
	if ((_unit isKindOf "Building")) then {
		...
		if (_isEngineer and (_type in USEC_CanDisassemble)) then {
			...
			_action = _unit addAction [format[localize "str_actions_medical_12",_typeVeh], "\dayz_code\actions\disassemble.sqf",[_unit,_inventory], 0, true, true];
			r_player_actions set [count r_player_actions,_action];
		};
		/*
		//Upgrade Wire
		if (_isEngineer and (_type == "usec_wire_cat1") and _hasWire) then {
			...
			_action = _unit addAction [format[localize "str_actions_medical_13",_typeVeh], "\dayz_code\actions\engineer_upgrade.sqf",[_unit,"ItemWire",_unitTo], 0, false, true];
			r_player_actions set [count r_player_actions,_action];
		};
	...
	if (count _crew > 0) then {
		...
		if (_patients > 0) then {
			if (!r_action_unload) then {
				...
				_vehType = typeOf _vehicle;
				_action = _vehicle addAction [format[localize "str_actions_medical_14",_vehType], "\dayz_code\medical\load\unLoad_act.sqf",[player,_vehicle], 0, false, true];
				r_player_actions set [count r_player_actions,_action];
			};
		} else {
			if (r_action_unload) then {
