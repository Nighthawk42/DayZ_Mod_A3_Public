...
CLASS("OO_EXTDB3")
	...
	PUBLIC FUNCTION("","constructor") { 
		...
		MEMBER("databasename", "");
		if (worldName isequalto "CUP_Chernarus_A3") then {
			MEMBER("inisectiondatabase", "Database");
		};
		if (worldName isequalto "taviana") then {
			MEMBER("inisectiondatabase", "Database_tavi");
		};
...
db_fnc_extdb3 = "new" call OO_EXTDB3;
if (worldName isequalto "CUP_Chernarus_A3") then {
	["setIniSectionDatabase", "Database"] call db_fnc_extdb3;
	["setDatabaseName", "dayz_a3_cherno"] call db_fnc_extdb3;
};
if (worldName isequalto "taviana") then {
	["setIniSectionDatabase", "Database_tavi"] call db_fnc_extdb3;
	["setDatabaseName", "dayz_a3_tavi"] call db_fnc_extdb3;
};
["setQueryType", "SQL"] call db_fnc_extdb3;

_result = "connect" call db_fnc_extdb3;
format ["%1", _result] call kk_fnc_log;	


private _query = "SELECT * FROM players LIMIT 10;";
...
db_fnc_update = {
	...
	if ((count _query) < 3) exitWith {"[] Error" call kk_fnc_log;};
	_result = ["executeQuery", _query] call db_fnc_extdb3;
	format ["[db_fnc_update] Query: %1#1111", _query] call kk_fnc_log;
	format ["[db_fnc_update] Result: %1#1111", _result] call kk_fnc_log;	
	_result;
};	
...
db_fnc_update2 = {
	...
	if ((count _query) < 3) exitWith {"[] Error" call kk_fnc_log;};
	_result = (["executeQuery", _query] call db_fnc_extdb3) select 0;
	format ["[db_fnc_update] Query: %1#1111", _query] call kk_fnc_log;
	format ["[db_fnc_update] Result: %1#1111", _result] call kk_fnc_log;	
	_result;
};	
