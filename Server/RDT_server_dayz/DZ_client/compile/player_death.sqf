...
[dayz_characterID,0,_body,_playerID,dayz_playerName] remoteExec ["server_playerDied",2];
...
if (count _array > 0) then {
	...
	[player,_source,_source] remoteExec ["Cytech_killfeed",2];
	if (!isNull _source) then {
		if (_source != player) then {
			...
			if (!_canHitFree and !_isBandit) then {
				...
				[_source,_humanity,_wait] remoteExec ["player_humanityChange",0];
			};
		};
	};
...
["epicFail", false, 2, false] call BIS_fnc_endMission;